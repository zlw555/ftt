...
int main(int argc, char *argv[]) {
	...
	seteuid(0);
	setgid(0);
	int sandBox = GetInt(@"folderSuffix", 1, PREFS);//Randy420 add
	...
	NSString *sandbox = GetNSString(@"folderPrefix", @"m", PREFS);//Randy420 edit
			...
			for (unsigned int choose = 0; choose < allPatchesCount; choose++) {
				...
				if (totalDump) {//Randy420 add
					...
					if (switchedOn || showAll)//Randy420 add
						printf("  %s%d%s: %s\n", switchedOn ? greenColor : greenColor, choose, resetColor, [allPatches[choose][@"name"] UTF8String]);//Randy420 edit
				}
			}
...
/*Randy420 add start*/
		if (description.length <= 4) {
			if (!adjustDescription){
				...
				text = local(@"PATCH_DESCRIPTION", @"请为您的补丁输入描述");
				printf("%s%s: %s", greenColor, text.UTF8String, resetColor);
				scanf(" %399[^\n]s", userDescription);
        ...
        /*Randy420 add end*/
                ...
                "Section: Tweaks\n"
                "Version: %@\n", title, name, author, description, author, version];//Randy420 edit
